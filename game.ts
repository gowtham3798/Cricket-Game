  ...
  displayRuns = () => {
    if(this.players < 11){
        ...
        if(this.balls < 7){
            ...
            runData.innerHTML = `${run}`

            if(run == 0){
                 document.getElementById(`t${this.team}${this.players}`)
                 ?.appendChild(runData)

                 totTabData.innerHTML = `<b style="color:red">${this.total}</b>`;
                 document.getElementById(`t${this.team}${this.players}`)?.appendChild(totTabData);
                 this.players++;
                 this.balls = 1;
                 this.total = 0;
            }
